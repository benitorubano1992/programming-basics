...
const getTeaSpoons=(num)=>{
    ...
    return  `${num} of teaspoons equals to ${numOfCup} of cups, ${numTableSpoons } tablespoons, ${numTeaSpoons} of teaspoons`
}
...
const validateUnit=(input)=>{
    const inputNumber = Number.parseInt(input,10);
        //console.log({input,inputNumber,isValid: Number.isInteger(inputNumber) && (inputNumber>= 0 && inputNumber <= 2)})

    return Number.isInteger(inputNumber) && (inputNumber>= 0 && inputNumber <= 2)
}


const validateNum =(input)=>{
    const inputNumber = Number.parseInt(input,10);
    return Number.isInteger(inputNumber) && (inputNumber>= 0)
}
...
async function main() {
    const rl = readline.createInterface({ input, output });
     try{
       ...
       const num = await getInput(rl,'num of unit',validateNum);
